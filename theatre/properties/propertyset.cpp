...
std::unique_ptr<PropertySet> PropertySet::Make(FolderObject &object) {
  ...
  const ConstantValueEffect *cfx =
      dynamic_cast<const ConstantValueEffect *>(&object);
  ...
  if (afx != nullptr) {
    ...
    throw std::runtime_error(
        "Unknown object type specified in call to PropertySet::Make()");
  }
  ps->_object = &object;
  return ps;
}
