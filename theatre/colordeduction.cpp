...
unsigned FitColor(Color color, unsigned red, unsigned green, unsigned blue) {
  const unsigned cr = color.Red() == 0 ? ControlValue::MaxUInt() : 255u * red / color.Red();
  const unsigned cg = color.Green() == 0 ? ControlValue::MaxUInt() : 255u * green / color.Green();
  const unsigned cb = color.Blue() == 0 ? ControlValue::MaxUInt() : 255u * blue / color.Blue();
  return std::min({ControlValue::MaxUInt(), cr, cg, cb});
}

// This returns an unnormalized value, only used for comparison
constexpr unsigned Mismatch(Color color, unsigned fit, unsigned red, unsigned green,
                            unsigned blue) {
  return (red - fit * color.Red()) + (green - fit * color.Green()) + (blue - fit * color.Blue());
}

void Add(Color color, unsigned fit, unsigned& red, unsigned& green, unsigned& blue) {
  red += fit * color.Red() / 255;
  green += fit * color.Green() / 255;
  blue += fit * color.Blue() / 255;
}

void Subtract(Color color, unsigned fit, unsigned& red, unsigned& green, unsigned& blue) {
...
unsigned DiffFit(Color a, Color b, unsigned red, unsigned green, unsigned blue, unsigned fit_a,
                 unsigned fit_b) {
  const unsigned char diff_r = std::max(0, static_cast<int>(b.Red()) - static_cast<int>(a.Red()));
  const unsigned char diff_g =
      std::max(0, static_cast<int>(b.Green()) - static_cast<int>(a.Green()));
  const unsigned char diff_b = std::max(0, static_cast<int>(b.Blue()) - static_cast<int>(a.Blue()));
  ...
  fit_diff = std::clamp(fit_diff, 0u, std::min(fit_a, ControlValue::MaxUInt() - fit_b));
...
void ApplyDiffFit(Color a, Color b, unsigned& red, unsigned& green, unsigned& blue, unsigned& fit_a,
                  unsigned& fit_b) {
...
void Solve2ColorFit(Color a, Color b, const ControlValue* rgb_values, ControlValue* fitted_values) {
  ...
  fit_a = std::min(ControlValue::MaxUInt(), fit_a + FitColor(a, red, green, blue));
...
void Normalized3ColorFit(Color a, Color b, Color c, const ControlValue* rgb_values,
                         ControlValue* fitted_values) {
  const unsigned m = std::max({rgb_values[0].UInt(), rgb_values[1].UInt(), rgb_values[2].UInt()});
  if (m == 0) {
    std::fill_n(fitted_values, 3, ControlValue(0));
  } else if (m != ControlValue::MaxUInt()) {
    const double ratio = static_cast<double>(ControlValue::MaxUInt()) / static_cast<double>(m);
    const ControlValue normalized_rgb[3] = {rgb_values[0] * ratio, rgb_values[1] * ratio,
                                            rgb_values[2] * ratio};
    Solve3ColorFit(a, b, c, normalized_rgb, fitted_values);
    const ControlValue max_fit = Max(fitted_values[0], fitted_values[1], fitted_values[2]);
    ...
    if (max_fit.UInt() == 0) {
      ...
      const double factor =
          ...
          static_cast<double>(Max(fitted_values[0], fitted_values[1], fitted_values[2]).UInt());
      for (size_t i = 0; i != 3; ++i) {
        fitted_values[i] = ControlValue(std::min(
            ControlValue::MaxUInt(), static_cast<unsigned>(factor * fitted_values[i].UInt())));
      }
    }
  }
}
