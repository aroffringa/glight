...
void Scene::OnSyncUpdate(double offsetInMS) {
  ...
  _startTimeInMS = StartTimeInMS() + (currentTime - StartTimeInMS() - offsetInMS) * 0.05;
}
...
void Scene::initPlayer() {
  if (!_audioFilename.empty()) {
    try {
      ...
      _audioPlayer =
          std::make_unique<system::AudioPlayer>(*_decoder, _management.Settings().audio_output);
...
ControlSceneItem *Scene::AddControlSceneItem(double offset_in_ms, Controllable &controllable,
                                             size_t input) {
  std::unique_ptr<ControlSceneItem> item = std::make_unique<ControlSceneItem>(controllable, input);
  ...
  if (std::find(controllables_.begin(), controllables_.end(), value) == controllables_.end()) {
...
BlackoutSceneItem &Scene::AddBlackoutItem(double offset_in_ms) {
  std::unique_ptr<BlackoutSceneItem> item = std::make_unique<BlackoutSceneItem>();
  item->SetOffsetInMS(offset_in_ms);
...
void Scene::Remove(SceneItem *item) {
  ...
  _items.erase(iter);
  if (ControlSceneItem *c_item = dynamic_cast<ControlSceneItem *>(item); c_item) {
...
void Scene::skipTo(double offsetInMS) {
  ...
  for (std::vector<SceneItem *>::iterator i = _startedItems.begin(); i != _startedItems.end();
       ++i) {
...
void Scene::RecalculateControllables() {
  ...
  for (const std::pair<const double, std::unique_ptr<SceneItem>> &item : _items) {
    if (ControlSceneItem *c_item = dynamic_cast<ControlSceneItem *>(item.second.get()); c_item) {
      controllables.emplace(std::make_pair(&c_item->GetControllable(), c_item->GetInput()));
    }
  }
