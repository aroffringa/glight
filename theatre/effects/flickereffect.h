...
private:
  virtual void mix(const ControlValue *values, unsigned *channelValues,
    ...
    if (values[0]) {
      ...
      _value.resize(count);
      double delta = 2.0 * (timing.TimeInMS() - _previousTime) * _speed /
      ...
      if (_independentOutputs) {
        for (size_t i = 0; i != Connections().size(); ++i) {
          Connections()[i].first->MixInput(
              ...
              ControlValue::MultiplyValues(values[0].UInt(), _value[i]));
        }
      } else {
        setConnectedInputs(
            ControlValue::MultiplyValues(values[0].UInt(), _value[0]));
      }
    }
  }
