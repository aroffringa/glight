...
protected:
	virtual void mix(const ControlValue* values, unsigned* channelValues, unsigned universe, const Timing& timing) final override
	{
		...
		{
			unsigned prevWritePos = (_bufferWritePos + _buffer.size() - 1) % _buffer.size();
			_buffer[prevWritePos].second.Set(
				ControlValue::Mix(_buffer[prevWritePos].second.UInt(), values[0].UInt(), ControlValue::Default) );
		}
		else {
			...
			if(_bufferWritePos == _bufferReadPos)
			{
				...
				std::copy_backward(_buffer.begin()+_bufferWritePos, _buffer.begin()+n, _buffer.begin()+_bufferReadPos);
			}
