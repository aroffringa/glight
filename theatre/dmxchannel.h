...
class DmxChannel{
	public:
		DmxChannel() : _universe(0), _channel(0), _defaultMixStyle(ControlValue::Default)
		{
		}
		DmxChannel(unsigned channel, unsigned universe)
  : _universe(universe), _channel(channel), _defaultMixStyle(ControlValue::Default)
		{
		}
		~DmxChannel()
		{
		...
		void SetDefaultMixStyle(ControlValue::MixStyle defaultMixStyle) { _defaultMixStyle = defaultMixStyle; }
