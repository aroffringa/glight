...
Fixture::Fixture(Theatre &theatre, const FixtureMode &type, const std::string &name)
  ...
  for (size_t ci = 0; ci != type.Functions().size(); ++ci) {
    ...
    if (function.FineChannelOffset()) fine_channel = base_channel + *function.FineChannelOffset();
...
DmxChannel Fixture::GetFirstChannel() const { return functions_.front()->MainChannel(); }
...
void Fixture::SetChannel(DmxChannel dmx_channel) {
  for (size_t i = 0; i != functions_.size(); ++i) {
    ...
    if (tf.FineChannelOffset()) {
      ff->SetChannel(dmx_channel + tf.DmxOffset(), dmx_channel + *tf.FineChannelOffset());
    } else {
...
double Fixture::GetBeamDirection(const ValueSnapshot &snapshot, size_t shape_index) const {
...
double Fixture::GetBeamTilt(const ValueSnapshot &snapshot, size_t shape_index) const {
