...
std::vector<Color> PresetCollection::InputColors(size_t) const {
  ...
  for (const std::unique_ptr<PresetValue>& preset : _presetValues) {
    ...
    if (fixture_control) {
      ...
      sum.red += color.Red() * value.UInt() / value.MaxUInt();
      sum.green += color.Green() * value.UInt() / value.MaxUInt();
      sum.blue += color.Blue() * value.UInt() / value.MaxUInt();
