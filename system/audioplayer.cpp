...
void AudioPlayer::open() {
  ...
  int rc = snd_pcm_open(&_handle, device_name_.c_str(), SND_PCM_STREAM_PLAYBACK, 0);
  if (rc < 0) throw AlsaError(std::string("snd_pcm_open() returned: ") + snd_strerror(rc));
  _isOpen = true;
  _isPlaying = true;
  ...
  while (!_isStopping && _decoder.HasMore()) {
    ...
    if (rc == -EPIPE) {
      ...
      std::cout << "Only " << rc << " frames were written in snd_pcm_writei().\n";
