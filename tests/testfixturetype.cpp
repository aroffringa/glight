...
BOOST_AUTO_TEST_CASE(ClassList) {
  const std::vector<enum FixtureType::FixtureClass> list = FixtureType::GetClassList();
  ...
  BOOST_CHECK_NO_THROW(FixtureType::ClassName((enum FixtureType::FixtureClass) std::numeric_limits<int>::max()));
  BOOST_CHECK_THROW(FixtureType::NameToClass("This is not a class! ~!@"), std::runtime_error);
}
...
Color testColor(enum FixtureType::FixtureClass cl, const std::vector<unsigned char>& values)
{
  Management management;
  const FixtureType& fixtureType =
      management.Theatre().AddFixtureType(cl);
...
BOOST_AUTO_TEST_CASE(GetColor_RGBAWUV) {
  const Color colorRed = testColor(FixtureType::RGBAWUVLight6Ch, {255, 0, 0, 0, 0, 0});
  BOOST_TEST(colorRed.Red() >= 64);
  BOOST_TEST(colorRed.Green() == 0);
  BOOST_TEST(colorRed.Blue() == 0);
  
  const Color colorGreen = testColor(FixtureType::RGBAWUVLight6Ch, {0, 255, 0, 0, 0, 0});
  BOOST_TEST(colorGreen.Red() == 0);
  BOOST_TEST(colorGreen.Green() >= 64);
  BOOST_TEST(colorGreen.Blue() == 0);
  
  const Color colorBlue = testColor(FixtureType::RGBAWUVLight6Ch, {0, 0, 255, 0, 0, 0});
  BOOST_TEST(colorBlue.Red() == 0);
  BOOST_TEST(colorBlue.Green() == 0);
  BOOST_TEST(colorBlue.Blue() >= 64);
  
  const Color colorAmber = testColor(FixtureType::RGBAWUVLight6Ch, {0, 0, 0, 255, 0, 0});
  BOOST_TEST(colorAmber.Red() >= 32);
  BOOST_TEST(colorAmber.Green() >= 24);
  BOOST_TEST(colorAmber.Blue() == 0);
  
  const Color colorWhite = testColor(FixtureType::RGBAWUVLight6Ch, {0, 0, 0, 0, 255, 0});
  BOOST_TEST(colorWhite.Red() >= 64);
  BOOST_TEST(colorWhite.Green() >= 64);
  BOOST_TEST(colorWhite.Blue() >= 64);
  
  const Color colorUV = testColor(FixtureType::RGBAWUVLight6Ch, {0, 0, 0, 0, 0, 255});
