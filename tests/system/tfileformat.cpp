...
void FillManagement(Management &management) {
  ...
  root.Add(management.GetTheatre().AddFixtureTypePtr(StockFixture::AyraTDCSunrise));

  // Make a group with the two fixtures
  management.AddFixtureGroup(root, "Ayra and ADJ");

  ObservingPtr<FixtureType> ft = management.GetTheatre().AddFixtureTypePtr(StockFixture::Rgbw);
  root.Add(ft);
  ...
  FolderObject &path_fc =
      management.GetObjectFromPath("The root folder/A subfolder/Control for RGBW fixture");
...
void CheckEqual(const Management &a, const Management &b) {
  ...
  BOOST_CHECK_EQUAL(a.RootFolder().Children()[0]->Name(), b.RootFolder().Children()[0]->Name());

  BOOST_REQUIRE_EQUAL(a.GetTheatre().FixtureTypes().size(), b.GetTheatre().FixtureTypes().size());
  ...
  BOOST_REQUIRE_EQUAL(a.GetTheatre().Fixtures().size(), b.GetTheatre().Fixtures().size());

  const Fixture &a_fixture = *a.GetTheatre().Fixtures()[0];
  ObservingPtr<FixtureControl> a_fixture_control = a.GetFixtureControl(a_fixture);
  ...
  BOOST_CHECK_EQUAL(a.GetFixtureControl(a_fixture).Get(),
                    &a.GetObjectFromPath("The root folder/A subfolder/Control for RGBW fixture"));

  BOOST_CHECK_EQUAL(a.GetSourceValue(*a_fixture_control, 0)->A().Value().UInt(), 0);
  BOOST_CHECK_EQUAL(a.GetSourceValue(*a_fixture_control, 1)->A().Value().UInt(), 0);
  BOOST_CHECK_EQUAL(a.GetSourceValue(*a_fixture_control, 2)->A().Value().UInt(),
                    ControlValue::MaxUInt());
  BOOST_CHECK_EQUAL(a.GetSourceValue(*a_fixture_control, 3)->A().Value().UInt(), 0);

  BOOST_REQUIRE_EQUAL(a.FixtureGroups().size(), b.FixtureGroups().size());
  for (size_t i = 0; i != a.FixtureGroups().size(); ++i) {
    BOOST_CHECK_EQUAL(a.FixtureGroups()[i]->Size(), b.FixtureGroups()[i]->Size());
    const std::vector<ObservingPtr<Fixture>> &fixtures = a.FixtureGroups()[i]->Fixtures();
    for (const ObservingPtr<Fixture> &fixture_a : fixtures) {
      ObservingPtr<Fixture> fixture_b = b.GetTheatre().GetFixturePtr(fixture_a->Name());
      BOOST_CHECK(b.FixtureGroups()[i]->Contains(fixture_b));
    }
  }

  const PresetCollection &readCollection = static_cast<const PresetCollection &>(
      a.GetObjectFromPath("The root folder/A subfolder/A preset collection"));
  ...
  BOOST_CHECK_EQUAL(readCollection.PresetValues()[0]->Value().UInt(), ControlValue::MaxUInt() / 2);
  BOOST_CHECK_EQUAL(&readCollection.PresetValues()[0]->GetControllable(), a_fixture_control.Get());
  BOOST_CHECK_EQUAL(readCollection.PresetValues()[0]->InputIndex(), 0);
  BOOST_CHECK_EQUAL(readCollection.PresetValues()[1]->Value().UInt(), ControlValue::MaxUInt());
  BOOST_CHECK_EQUAL(&readCollection.PresetValues()[1]->GetControllable(), a_fixture_control.Get());
  BOOST_CHECK_EQUAL(readCollection.PresetValues()[1]->InputIndex(), 3);
  BOOST_CHECK_NE(a.GetSourceValue(readCollection, 0), nullptr);
  BOOST_CHECK_EQUAL(&a.GetSourceValue(readCollection, 0)->GetControllable(), &readCollection);
  BOOST_CHECK_EQUAL(a.GetSourceValue(readCollection, 0)->InputIndex(), 0);
  BOOST_CHECK_EQUAL(a.GetSourceValue(readCollection, 0)->A().Value().UInt(), 42);

  const Chase &readChase =
      static_cast<const Chase &>(a.GetObjectFromPath("The root folder/A subfolder/A chase"));
  BOOST_CHECK_EQUAL(readChase.GetSequence().size(), 2);
  BOOST_CHECK_EQUAL(readChase.GetSequence()[0].GetControllable(), &readCollection);
  ...
  BOOST_CHECK_EQUAL(readEffect->Connections()[1].first, a_fixture_control.Get());
  BOOST_CHECK_EQUAL(readEffect->Connections()[1].second, 1);

  BOOST_REQUIRE_EQUAL(a.Controllables().size(), b.Controllables().size());
  for (size_t controllable_index = 0; controllable_index != a.Controllables().size();
       ++controllable_index) {
    ...
    BOOST_CHECK_EQUAL(controllable_a.NConnections(), controllable_b.NConnections());
    if (const Scene *scene_a = dynamic_cast<const Scene *>(&controllable_a); scene_a) {
      const Scene *scene_b = dynamic_cast<const Scene *>(&controllable_b);
      BOOST_CHECK(scene_b);
      BOOST_CHECK_EQUAL(scene_a->SceneItems().size(), scene_b->SceneItems().size());
      auto iterator_b = scene_b->SceneItems().begin();
      for (const auto &pair_a : scene_a->SceneItems()) {
        BOOST_CHECK(typeid(pair_a.second.get()) == typeid(iterator_b->second.get()));
        if (BlackoutSceneItem *blackout_a = dynamic_cast<BlackoutSceneItem *>(pair_a.second.get());
            blackout_a) {
          ...
          BOOST_CHECK_CLOSE_FRACTION(blackout_a->FadeSpeed(), blackout_b->FadeSpeed(), 1e-5);
        }
        ++iterator_b;
      }
    }
...
BOOST_AUTO_TEST_CASE(ReadAndWrite) {
  ...
  std::vector<std::unique_ptr<glight::uistate::FaderSetState>> &setups = guiState.FaderSets();
  ...
  glight::uistate::FaderState &state =
      *setup->faders.emplace_back(std::make_unique<glight::uistate::FaderState>());
  state.SetSourceValues({write_management.SourceValues()[0].get()});

  glight::system::Write("tmp-testfileformat.gshow", write_management, &guiState);
