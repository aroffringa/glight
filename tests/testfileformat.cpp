...
BOOST_AUTO_TEST_CASE( ReadAndWrite )
{
	...
	BOOST_CHECK_EQUAL(management.RootFolder().Children()[0]->Name(), "A subfolder");
	BOOST_CHECK_EQUAL(management.Theatre().Fixtures().size(), 1);
	
	Fixture& readFixture = *management.Theatre().Fixtures()[0];
	FixtureControl& readFixtureControl = management.GetFixtureControl(readFixture);
	...
	BOOST_CHECK_EQUAL(management.GetPresetValue(readFixtureControl, 0)->Value().UInt(), 0);
	BOOST_CHECK_EQUAL(management.GetPresetValue(readFixtureControl, 1)->Value().UInt(), 0);
	BOOST_CHECK_EQUAL(management.GetPresetValue(readFixtureControl, 2)->Value().UInt(), ControlValue::MaxUInt());
	BOOST_CHECK_EQUAL(management.GetPresetValue(readFixtureControl, 3)->Value().UInt(), 0);
	
	PresetCollection& readCollection = 
		static_cast<PresetCollection&>(management.GetObjectFromPath("The root folder/A subfolder/A preset collection"));
	...
	BOOST_CHECK_EQUAL(&readCollection.PresetValues()[0]->Controllable(), &readFixtureControl);
	...
	BOOST_CHECK_EQUAL(&readCollection.PresetValues()[1]->Controllable(), &readFixtureControl);
	...
	BOOST_CHECK_EQUAL(&management.GetPresetValue(readCollection, 0)->Controllable(), &readCollection);
	BOOST_CHECK_EQUAL(management.GetPresetValue(readCollection, 0)->InputIndex(), 0);
	BOOST_CHECK_EQUAL(management.GetPresetValue(readCollection, 0)->Value().UInt(), 42);
	
	Chase& readChase = 
		static_cast<Chase&>(management.GetObjectFromPath("The root folder/A subfolder/A chase"));
	...
	AudioLevelEffect* readEffect =
		dynamic_cast<AudioLevelEffect*>(&management.GetObjectFromPath("The root folder/Effect folder/An audio effect"));
