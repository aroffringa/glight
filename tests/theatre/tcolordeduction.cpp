...
unsigned Residual(ControlValue value, ControlValue result[2], unsigned char color_a_value,
                  unsigned char color_b_value) {
  const unsigned term = (result[0].UInt() * color_a_value + result[1].UInt() * color_b_value) / 255;
  if (value.UInt() >= term)
...
void Fit2ResidualTest(Color color_a, Color color_b, const ControlValue rgb[3]) {
  ...
  for (size_t i = 0; i != 3; ++i) truncated_rgb[i] = Min(ControlValue::Max(), rgb[i]);
  ...
  const unsigned residual_r = Residual(truncated_rgb[0], result, color_a.Red(), color_b.Red());
  BOOST_CHECK_LT(residual_r, tolerance);

  const unsigned residual_g = Residual(truncated_rgb[1], result, color_a.Green(), color_b.Green());
  BOOST_CHECK_LT(residual_g, tolerance);

  const unsigned residual_b = Residual(truncated_rgb[2], result, color_a.Blue(), color_b.Blue());
  BOOST_CHECK_LT(residual_b, tolerance);
}

void Fit3ResidualTest(Color color_a, Color color_b, Color color_c, const ControlValue rgb[3]) {
  ControlValue truncated_rgb[3];
  for (size_t i = 0; i != 3; ++i) truncated_rgb[i] = Min(ControlValue::Max(), rgb[i]);
  ...
  const unsigned tolerance = 256;
  const unsigned residual_r = truncated_rgb[0].UInt() -
                              (result[0].UInt() * color_a.Red() + result[1].UInt() * color_b.Red() +
                               result[2].UInt() * color_c.Red()) /
                                  255;
  BOOST_CHECK_LT(residual_r, tolerance);

  const unsigned residual_g = truncated_rgb[1].UInt() - (result[0].UInt() * color_a.Green() +
                                                         result[1].UInt() * color_b.Green() +
                                                         result[2].UInt() * color_c.Green()) /
                                                            255;
  BOOST_CHECK_LT(residual_g, tolerance);

  const unsigned residual_b = truncated_rgb[2].UInt() - (result[0].UInt() * color_a.Blue() +
                                                         result[1].UInt() * color_b.Blue() +
...
BOOST_AUTO_TEST_CASE(fit_2_colors_full) {
  const ControlValue rgb[3] = {ControlValue::Max(), ControlValue::Max(), ControlValue::Max()};
  ControlValue result[2];
...
BOOST_AUTO_TEST_CASE(fit_2_colors_complex_a) {
  const ControlValue rgb[3] = {ControlValue::Max(), ControlValue::Max() / 4, ControlValue::Zero()};
...
BOOST_AUTO_TEST_CASE(solve_2_color_fit_complex_b) {
  const ControlValue rgb[3] = {ControlValue::Max(), ControlValue::Max(), ControlValue::Max() / 2};
...
BOOST_AUTO_TEST_CASE(fit_2_colors_oversaturated) {
  const ControlValue rgb[3] = {ControlValue::Max(), ControlValue::Max(), ControlValue::Zero()};
