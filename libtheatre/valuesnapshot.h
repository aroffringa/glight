...
class ValueSnapshot
{
	public:
		...
		{
			for(const std::unique_ptr<ValueUniverseSnapshot>& snapshot : source._universeValues)
		...
		ValueSnapshot &operator=(const ValueSnapshot &rhs)
		{
			...
			for(const std::unique_ptr<ValueUniverseSnapshot>& snapshot : rhs._universeValues)
				_universeValues.emplace_back(new ValueUniverseSnapshot(*snapshot));
			return *this;
		}
