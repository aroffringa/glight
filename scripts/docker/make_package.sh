...
sudo docker run --mount src=${glightdir}/build/package,target=/package,type=bind glight-package-ubuntu24 /usr/bin/bash -c "mv /build/glight-0.9.3-Linux.deb /package/glight-0.9.3-Ubuntu24.deb"
...
mkdir -p ${glightdir}/build/package
sudo docker run --mount src=${glightdir}/build/package,target=/package,type=bind glight-package-ubuntu22 /usr/bin/bash -c "mv /build/glight-0.9.3-Linux.deb /package/glight-0.9.3-Ubuntu22.deb"
echo Package was written to ${glightdir}/build/package/
