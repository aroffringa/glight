...
PresetCollectionWindow::PresetCollectionWindow(PresetCollection& presetCollection, Management &management, EventTransmitter& eventHub) :
	PropertiesWindow(),
	...
	_eventHub(eventHub)
{
	_changeManagementConnection =
		eventHub.SignalChangeManagement().connect(sigc::mem_fun(*this, &PresetCollectionWindow::onChangeManagement));
	_updateControllablesConnection =
		eventHub.SignalUpdateControllables().connect(sigc::mem_fun(*this, &PresetCollectionWindow::onUpdateControllables));
	
	set_title("glight - " + presetCollection.Name());
	
	_inputSelector.SignalSelectionChange().connect(sigc::mem_fun(*this, &PresetCollectionWindow::onInputSelectionChanged));
	...
	_addPresetButton.signal_clicked().connect(sigc::mem_fun(*this, &PresetCollectionWindow::onAddPreset));
	_buttonBox.pack_start(_addPresetButton, false, false, 4);
	
	_removePresetButton.set_image_from_icon_name("go-previous");
	_removePresetButton.signal_clicked().connect(sigc::mem_fun(*this, &PresetCollectionWindow::onRemovePreset));
	...
	fillPresetsList();
	_presetsView.get_selection()->signal_changed().connect(sigc::mem_fun(*this, &PresetCollectionWindow::onSelectedPresetChanged));
...
void PresetCollectionWindow::load()
{
	fillPresetsList();
}
...
void PresetCollectionWindow::fillPresetsList()
{
	...
	{
		const std::unique_ptr<PresetValue>& pValue = _presetCollection->PresetValues()[i];
		Gtk::TreeModel::iterator iter = _presetsStore->append();
		Gtk::TreeModel::Row row = *iter;
		row[_presetListColumns._control] = pValue->Controllable().InputName(pValue->InputIndex());
