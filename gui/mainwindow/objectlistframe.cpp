...
ObjectListFrame::ObjectListFrame(MainWindow &parentWindow)
    : Gtk::Box(Gtk::Orientation::VERTICAL), _list(), _parentWindow(parentWindow), _nameFrame() {
  _list.SignalSelectionChange().connect(
      sigc::mem_fun(*this, &ObjectListFrame::onSelectedObjectChanged));
  _list.SignalObjectActivated().connect([&](ObservingPtr<theatre::FolderObject> object) {
  ...
  menu.AddCurrentPreset.connect([&]() { mainwindow::NewPresetFromCurrent(_list); });
...
PropertiesWindow &ObjectListFrame::OpenPropertiesWindow(theatre::FolderObject &object) {
