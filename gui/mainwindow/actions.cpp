...
PropertiesWindow &OpenPropertiesWindow(ObjectWindowList<PropertiesWindow> &property_windows,
                                       FolderObject &object, Gtk::Window &parent) {
  ...
  if (!window) {
    ...
    } else if (theatre::Effect *effect = dynamic_cast<theatre::Effect *>(&object); effect) {
      new_window = std::make_unique<EffectPropertiesWindow>(*effect);
    } else if (theatre::FixtureGroup *group = dynamic_cast<theatre::FixtureGroup *>(&object);
               group) {
      new_window = std::make_unique<windows::GroupWindow>(*group);
    } else if (PresetCollection *presetCollection = dynamic_cast<PresetCollection *>(&object);
               presetCollection) {
      new_window = std::make_unique<PresetCollectionWindow>(*presetCollection);
    } else if (TimeSequence *timeSequence = dynamic_cast<TimeSequence *>(&object); timeSequence) {
      new_window = std::make_unique<TimeSequencePropertiesWindow>(*timeSequence);
    }
    if (new_window) {
...
void NewEmptyPreset(ObjectBrowser &browser, ObjectWindowList<PropertiesWindow> &property_windows,
                    Gtk::Window &parent) {
  ...
  ObservingPtr<theatre::PresetCollection> preset_collection = management.AddPresetCollectionPtr();
...
void NewChase(std::unique_ptr<Gtk::Dialog> &dialog, ObjectBrowser &browser,
              ObjectWindowList<PropertiesWindow> &property_windows, Gtk::Window &parent) {
  ...
  chase_dialog.SignalNewChase().connect(
      [&dialog, &browser, &property_windows, &parent](theatre::Chase &new_chase) {
...
void NewTimeSequence(ObjectBrowser &browser, ObjectWindowList<PropertiesWindow> &property_windows,
                     Gtk::Window &parent) {
  Management &management = Instance::Management();
  std::unique_lock<std::mutex> lock(management.Mutex());
  Folder &parent_folder = browser.SelectedFolder();
  ObservingPtr<theatre::TimeSequence> time_sequence = management.AddTimeSequencePtr();
...
void NewEffect(theatre::EffectType effect_type, ObjectBrowser &browser,
  ...
  std::unique_lock<std::mutex> lock(management.Mutex());
  Folder &parent_folder = browser.SelectedFolder();
  effect->SetName(parent_folder.GetAvailableName(EffectTypeToName(effect_type)));
  ObservingPtr<Effect> added = management.AddEffectPtr(std::move(effect), parent_folder);
...
void NewFolder(ObjectBrowser &browser) {
  ...
  Management &management = Instance::Management();
  std::unique_lock<std::mutex> lock(management.Mutex());
  Folder &folder = management.AddFolder(parent_folder, parent_folder.GetAvailableName("Folder"));
  lock.unlock();
