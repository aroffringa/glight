	...
	_isUpdating(false)
{
	_audioWidget.SignalClicked().
		connect(sigc::mem_fun(*this, &SceneFrame::onAudioWidgetClicked));
	...
	_changeAudioButton.signal_clicked().
			connect(sigc::mem_fun(*this, &SceneFrame::onChangeAudioButtonPressed));
	...
	_startButton.signal_clicked().
		connect(sigc::mem_fun(*this, &SceneFrame::onStartButtonPressed));
	_sceneItemUButtonBox.pack_start(_startButton);
	_startButton.show();

	_startSelectionButton.signal_clicked().
		connect(sigc::mem_fun(*this, &SceneFrame::onStartSelectionButtonPressed));
	_sceneItemUButtonBox.pack_start(_startSelectionButton);
	_startSelectionButton.show();

	_stopButton.signal_clicked().
		connect(sigc::mem_fun(*this, &SceneFrame::onStopButtonPressed));
	_sceneItemUButtonBox.pack_start(_stopButton);
	_stopButton.show();

	_key1Button.signal_clicked().
		connect(sigc::mem_fun(*this, &SceneFrame::onKey1ButtonPressed));
	_sceneItemUButtonBox.pack_start(_key1Button);
	_key1Button.show();

	_createControlItemButton.signal_clicked().
		connect(sigc::mem_fun(*this, &SceneFrame::onCreateControlItemButtonPressed));
	...
	_setEndTimeButton.signal_clicked().
		connect(sigc::mem_fun(*this, &SceneFrame::onSetEndTimeButtonPressed));
	_setEndTimeButton.set_sensitive(false);
	_sceneItemUButtonBox.pack_start(_setEndTimeButton);
	_setEndTimeButton.show();

	_removeButton.signal_clicked().
		connect(sigc::mem_fun(*this, &SceneFrame::onRemoveButtonPressed));
	...
	_startScale.signal_value_changed().connect(sigc::mem_fun(*this, &SceneFrame::onScalesChanged));
	...
	_endScale.signal_value_changed().connect(sigc::mem_fun(*this, &SceneFrame::onScalesChanged));
	...
	_timeoutConnection = Glib::signal_timeout().connect( sigc::mem_fun(*this, &SceneFrame::onTimeout), 20);
}
...
{
	_sceneItemsListModel =
    Gtk::ListStore::create(_sceneItemsListColumns);
	...
	_sceneItemsListView.get_selection()->signal_changed().connect(sigc::mem_fun(*this, &SceneFrame::onSelectedSceneItemChanged));
...
{
	_controllablesListModel =
    Gtk::ListStore::create(_controllablesListColumns);
...
void SceneFrame::fillSceneItemList()
{
	...
	if(_selectedScene != 0)
	{
		...
		const std::multimap<double, std::unique_ptr<SceneItem>>
			&items = _selectedScene->SceneItems();
		for(const std::pair<const double, std::unique_ptr<SceneItem>>& item : items)
		{
...
void SceneFrame::setSceneItemListRow(SceneItem *sceneItem, Gtk::TreeModel::Row row)
{
	...
	if(dynamic_cast<const KeySceneItem*>(sceneItem) == 0)
	{
		...
		const ControlSceneItem *csi = dynamic_cast<const ControlSceneItem*>(sceneItem);
		if(csi != 0)
		{
...
void SceneFrame::fillControllablesList()
{
	...
	const std::vector<std::unique_ptr<Controllable>>&
		controllable = _management.Controllables();
...
void SceneFrame::addKey(enum KeySceneItem::Level level)
{
	std::unique_lock<std::mutex> lock(_management.Mutex());
	KeySceneItem *key =
		_selectedScene->AddKeySceneItem(_management.GetOffsetTimeInMS() - _selectedScene->StartTimeInMS());
	key->SetLevel(level);
	lock.unlock();
...
void SceneFrame::onKey1ButtonPressed()
{
...
{
	Gtk::TreeModel::iterator activeControllable = _controllables1ComboBox.get_active();
	if(activeControllable)
	{
		...
		{
			SceneItem
				*selItem = (*_sceneItemsListModel->get_iter(*pathPtr))[_sceneItemsListColumns._item],
				*nextItem = 0;
...
void SceneFrame::onRemoveButtonPressed()
{
	...
	for(std::vector<Gtk::TreeModel::Path>::const_iterator pathPtr = pathHandle.begin();
		pathPtr != pathHandle.end(); ++pathPtr)
	{
		SceneItem *item = (*_sceneItemsListModel->get_iter(*pathPtr))[_sceneItemsListColumns._item];
...
{
	std::unique_lock<std::mutex> lock(_management.Mutex());
	if(_selectedScene!=0 && _selectedScene->IsPlaying())
	{
...
{
	std::unique_lock<std::mutex> lock(_management.Mutex());
	if(_selectedScene!=0 && _selectedScene->AudioFile() != _audioFile)
	{
