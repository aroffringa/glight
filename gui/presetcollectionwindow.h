...
public:
	PresetCollectionWindow(PresetCollection& presetCollection, class Management& management, class EventTransmitter& eventHub);
	~PresetCollectionWindow();

	class FolderObject& GetObject() final override;
	class PresetCollection& GetPresetCollection() { return *_presetCollection; }
...
private:
	...
	{
		PresetListColumns()
			{ add(_control); add(_value); add(_presetIndex); }
