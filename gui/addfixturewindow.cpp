...
AddFixtureWindow::AddFixtureWindow(EventTransmitter* eventHub, Management& management) :
	_typeLabel("Type:"),
	_countLabel("Count:"),
	_countEntry(),
	_decCountButton("-"), _incCountButton("+"),
	_cancelButton("Cancel"), _addButton("Add"),
	_eventHub(*eventHub),
...
void AddFixtureWindow::onAdd()
{
	...
	if(count > 0)
	{
		...
		FixtureType* type = dynamic_cast<FixtureType*>(_management->RootFolder().GetChildIfExists(className));
		if(!type)
		{
