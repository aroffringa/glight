...
MultiControllableSelectionDialog::MultiControllableSelectionDialog() {
  update_connection_ = Instance::Events().SignalUpdateControllables().connect(
      sigc::mem_fun(*this, &MultiControllableSelectionDialog::OnUpdateControllables));

  set_title("glight - select controllables");

  object_browser_.SignalSelectionChange().connect(
      sigc::mem_fun(*this, &MultiControllableSelectionDialog::OnObjectSelectionChanged));
  ...
  selection_view_.get_selection()->signal_changed().connect(
      sigc::mem_fun(*this, &MultiControllableSelectionDialog::OnViewSelectionChanged));
  selection_scrolled_window_.set_child(selection_view_);

  selection_scrolled_window_.set_size_request(200, 200);
  selection_scrolled_window_.set_policy(Gtk::PolicyType::NEVER, Gtk::PolicyType::AUTOMATIC);
...
bool MultiControllableSelectionDialog::IsSelected(const theatre::Controllable& controllable) const {
...
std::vector<theatre::Controllable*> MultiControllableSelectionDialog::GetSelection() const {
...
void MultiControllableSelectionDialog::SetSelection(const std::vector<Controllable*>& selection) {
...
void MultiControllableSelectionDialog::OnViewSelectionChanged() {
  const Gtk::TreeModel::iterator iter = selection_view_.get_selection()->get_selected();
  remove_button_.set_sensitive(static_cast<bool>(iter));
}

void MultiControllableSelectionDialog::OnAdd() {
  const std::vector<ObservingPtr<theatre::FolderObject>> objects = object_browser_.Selection();
...
void MultiControllableSelectionDialog::OnRemove() {
  Gtk::TreeModel::iterator selection_iter = selection_view_.get_selection()->get_selected();
  if (selection_iter) {
