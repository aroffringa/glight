#ifndef AVOID_RECURSION_H
#define AVOID_RECURSION_H

class AvoidRecursion
{
public:
	AvoidRecursion() : _isTaken(false) { }
	
	class Token 
	{
	public:
		Token(AvoidRecursion& ar) : _isTaken(&ar._isTaken)
		{
#ifndef NDEBUG
			if(ar._isTaken)
...
#ifndef NDEBUG
			if(!_owned)
				throw std::runtime_error("Logical error: Release() was called twice for token");
			if(!ar._isTaken)
				throw std::runtime_error("Logical error: token was released from wrong scope");
#endif
			if(_owned)
