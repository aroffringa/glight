...
class AudioWidget : public Gtk::DrawingArea {
	public:
		...
		double Position() const { return _centerPosition * _chunkSize / (44.100 * 4.0); }
		...
		sigc::signal<void, double> SignalClicked()
		{
	...
	private:
		...
		bool _isUpToDate;
		std::vector<int>
			_audioDataMax, _audioDataMin, _audioDataStdDev;
		...
		{
			if((_width!=get_width() || _height!=get_height()) && _height>0 && _width>0)
			{
				initialize();
				_isUpToDate = false;
			}
