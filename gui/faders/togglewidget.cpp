...
ToggleWidget::ToggleWidget(class Management &management, EventTransmitter& eventHub, char key) :
	...
	_holdUpdates(false)
{
	...
	_flashButton.signal_button_press_event().connect(sigc::mem_fun(*this, &ToggleWidget::onFlashButtonPressed), false);
	_flashButton.set_events(Gdk::BUTTON_PRESS_MASK);
	_flashButton.signal_button_release_event().connect(sigc::mem_fun(*this, &ToggleWidget::onFlashButtonReleased), false);
	...
	_onCheckButton.signal_clicked().
		connect(sigc::mem_fun(*this, &ToggleWidget::onOnButtonClicked));
...
void ToggleWidget::FullOn()
{
	_onCheckButton.set_active(true);
}

void ToggleWidget::FullOff()
{
	_onCheckButton.set_active(false);
}

void ToggleWidget::ChangeManagement(class Management& management, bool moveSliders)
{
	if(_preset == nullptr)
	{
		_management = &management;
