...
class FixtureSelection {
 public:
  ...
  void SetSelection(const std::vector<system::ObservingPtr<theatre::Fixture>> &selection) {
    _selection = selection;
    _signalChange.emit();
  }
  void SetSelection(std::vector<system::ObservingPtr<theatre::Fixture>> &&selection) {
  ...
  void UpdateAfterDelete() {
    const auto iter = std::remove(_selection.begin(), _selection.end(), nullptr);
