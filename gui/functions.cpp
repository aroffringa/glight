...
void OpenColorDialog(std::unique_ptr<Gtk::Dialog> &dialog, Gtk::Window &parent,
  ...
  dialog = std::make_unique<Gtk::ColorChooserDialog>("Select color for fixture");
  Gtk::ColorChooserDialog &color_dialog = static_cast<Gtk::ColorChooserDialog &>(*dialog);
  ...
  for (const theatre::Color &color : color_set) {
    colors.emplace_back().set_rgba(color.RedRatio(), color.GreenRatio(), color.BlueRatio());
  }
...
void AssignFader(theatre::Controllable &controllable) {
  if (controllable.NInputs() == 1) {
    ...
    glight::uistate::FaderState *fader = Instance::State().GetFirstUnassignedFader();
    if (fader) {
      theatre::SourceValue *source_value = management.GetSourceValue(controllable, 0);
