...
public:
		...
		_fadeToBlackRB("To black")
	{
		...
		_noneRB.signal_clicked().connect([&]() { _signalChanged.emit(Transition::None); } );
		_topBox.pack_start(_noneRB);
		
		_fadeRB.set_group(transTypeGroup);
		_fadeRB.signal_clicked().connect([&]() { _signalChanged.emit(Transition::Fade); } );
		_topBox.pack_start(_fadeRB);
		
		_fadeThroughBlackRB.set_group(transTypeGroup);
		_fadeThroughBlackRB.signal_clicked().connect([&]() { _signalChanged.emit(Transition::FadeThroughBlack); } );
		_topBox.pack_start(_fadeThroughBlackRB);
		
		_erraticRB.set_group(transTypeGroup);
		_erraticRB.signal_clicked().connect([&]() { _signalChanged.emit(Transition::Erratic); } );
		_topBox.pack_start(_erraticRB);
		
		_blackRB.set_group(transTypeGroup);
		_blackRB.signal_clicked().connect([&]() { _signalChanged.emit(Transition::Black); } );
		_bottomBox.pack_start(_blackRB);
		
		_fadeFromBlackRB.set_group(transTypeGroup);
		_fadeFromBlackRB.signal_clicked().connect([&]() { _signalChanged.emit(Transition::FadeFromBlack); } );
		_bottomBox.pack_start(_fadeFromBlackRB);
		
		_fadeToBlackRB.set_group(transTypeGroup);
		_fadeToBlackRB.signal_clicked().connect([&]() { _signalChanged.emit(Transition::FadeToBlack); } );
	...
	sigc::signal<void(enum Transition::Type)>& SignalChanged() { return _signalChanged; }
...
private:
	...
	Gtk::RadioButton
		_noneRB, _fadeRB,
		_fadeThroughBlackRB, _erraticRB,
		_blackRB,
