...
ControlButton::ControlButton(const std::string& label) : label_(label) {
  ...
  flash_gesture->signal_pressed().connect([this, g = flash_gesture.get()](int, double, double) {
    const int button = g->get_current_button();
    if (signal_button_ == 0 || signal_button_ == button) signal_press_(button);
  });
  flash_gesture->signal_released().connect([this, g = flash_gesture.get()](int, double, double) {
    const int button = g->get_current_button();
    if (signal_button_ == 0 || signal_button_ == button) signal_release_(button);
  });
