...
AudioWidget::AudioWidget()
	: _centerPosition(0), _renderStartPosition(0), _buffer(0), _width(0), _height(0), _isUpToDate(false), _chunkSize(800), _chunkBuffer(_chunkSize), _scene(0)
{
	add_events(Gdk::BUTTON_PRESS_MASK);

	signal_button_press_event().connect(sigc::mem_fun(*this, &AudioWidget::onButtonPressed));
...
void AudioWidget::draw(const Cairo::RefPtr<Cairo::Context>& context)
{
	...
	if(_buffer)
	{
		...
		for(int x=0;x<renderWidth;++x) {
			...
			guint8* xa = data + x * 3;
			int
				yStart = (_height/2) - (_audioDataMax[xDataPos] * _height) / 65536,
				yStd1 = (_height/2) - (_audioDataStdDev[xDataPos] * _height) / 65536,
				yStd2 = (_audioDataStdDev[xDataPos] * _height) / 65536 + (_height/2),
		...
		verticalLine(data, rowStride, _centerPosition - _renderStartPosition - 1, 255, 0, 0);
		verticalLine(data, rowStride, _centerPosition - _renderStartPosition, 255, 0, 0);
		verticalLine(data, rowStride, _centerPosition - _renderStartPosition + 1, 255, 0, 0);
...
void AudioWidget::UpdateKeys()
{
	...
	if(_scene)
	{
		...
		for(const std::pair<const double, std::unique_ptr<SceneItem>>& p : items)
		{
			...
			if(key != nullptr)
				_keys.insert(std::pair<int, KeyType>((int) round(item->OffsetInMS() * 44.100 * 4.0 / _chunkSize), KeyStart));
			else
				_keys.insert(std::pair<int, KeyType>((int) round(item->OffsetInMS() * 44.100 * 4.0 / _chunkSize), ItemStart));
		}
	}
	_isUpToDate = false;
	queue_draw();
